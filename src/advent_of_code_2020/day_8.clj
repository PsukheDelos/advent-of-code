...
(defn execute-boot-code [instructions]
  (loop [acc 0
    ...
    (let [[op sign n] (get instructions idx)]

      (cond
        (contains? processed-instructions idx)
        acc

        (= :acc op)
        (recur (long (sign acc n)) (inc idx) (conj processed-instructions idx))

        (= :jmp op)
        (recur acc (long (sign idx n)) (conj processed-instructions idx))

        (= :nop op)
        (recur acc (inc idx) (conj processed-instructions idx))))))


(defn fix-boot-code [instructions acc idx processed-instructions swapped?]
  (let [[op sign n] (get instructions idx)]

    (cond
      (contains? processed-instructions idx)                ;; we've hit a loop
      nil

      (= nil op)                                            ;; we have completed
      acc

      (= :acc op)
      (fix-boot-code instructions (sign acc n) (inc idx) (conj processed-instructions idx) swapped?)

      (= :jmp op)
      (if swapped?
        (fix-boot-code instructions acc (sign idx n) (conj processed-instructions idx) swapped?)
        (or (fix-boot-code instructions acc (sign idx n) (conj processed-instructions idx) false)
            (fix-boot-code instructions acc (inc idx) (conj processed-instructions idx) true)))

      (= :nop op)
      (if swapped?
        (fix-boot-code instructions acc (inc idx) (conj processed-instructions idx) swapped?)
        (or (fix-boot-code instructions acc (inc idx) (conj processed-instructions idx) false)
            (fix-boot-code instructions acc (sign idx n) (conj processed-instructions idx) true))))))
