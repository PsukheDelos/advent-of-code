...
(defn op-instruction [op-code i in mode-1 mode-2 mode-3]
  (assoc in (get-value (+ i 3) in mode-3)
            ((case (int op-code)
               1 +
               2 *)
             (get-value (+ i 1) in mode-1)
             (get-value (+ i 2) in mode-2))))


(defn in-instruction [i in mode]
  (println "Input: ")
  (assoc in (get-value (+ i 1) in mode) (Integer/parseInt (read-line))))

(defn out-instruction [i in mode]
  (println (get-value (+ i 1) in mode))
  in)

(defn parameter-instruction [i in]
  (let [parameter-code (int (nth in i))
        op-code (int (mod parameter-code 100))
        [mode-1
         mode-2
         mode-3] (->> (-> parameter-code
                          str
                          reverse)
                      (drop 2)
                      (map #(Character/digit (char %) 10)))]
    (case op-code
      (1 2) [(op-instruction op-code i in mode-1 mode-2 mode-3) (+ i 4)]
      3 [(in-instruction i in mode-1) (+ i 2)]
      4 [(out-instruction i in mode-1) (+ i 2)])))
...
(defn run-intcode
  ...
  ([input]
   (loop [[in i] [input 0]]
     (let [instruction (int (nth in i))]
       (cond
         (= instruction 99) in
         (= instruction 1) (recur [(op-instruction 1 i in 0 0 1) (+ i 4)])
         (= instruction 2) (recur [(op-instruction 2 i in 0 0 1) (+ i 4)])
         (= instruction 3) (recur [(in-instruction i in ) (+ i 2)])
         (= instruction 4) (recur [(out-instruction i in 0) (+ i 2)])
         :else (recur (parameter-instruction i in))
         )))))

#_(println (run-intcode [1002, 4, 3, 4, 33]))

#_(println (run-intcode [1, 0, 0, 0, 99]))

#_(println (run-intcode [1, 0, 0, 0, 99]))

#_(run-intcode [3, 0, 4, 0, 99])
#_(println (run-intcode [3, 0, 4, 0, 99]))
#_(println (run-intcode [1002, 4, 3, 4, 33]))
(run-intcode day-5-inputs)
