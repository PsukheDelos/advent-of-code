...
(defn spy [exp]
  (do (println exp #_(str/join ": " ["Spy" (str (macroexpand exp)) (type exp) exp]))
      exp))
...
(defn in-instruction [i in]
  (println "Input: ")
  (assoc in (nth in (+ i 1)) (Integer/parseInt (read-line))))

(defn out-instruction [i in]
  (print (->> (+ i 1)
              (nth in)
              (nth in)))
  in)

#_#_(defn in-instruction [i in mode]
  (println "Input: ")
  (assoc in (get-value (+ i 1) in mode) (Integer/parseInt (read-line))))

(defn out-instruction [i in mode]
  (print (get-value (+ i 1) in mode))
  in)

(defn parameter-instruction [i in]
  (let [parameter-code (int (nth in i))
        op-code (mod parameter-code 100)
        [mode-1
         mode-2
    ...
    (case op-code
      ...
      3 [(in-instruction i in) (+ i 2)]
      4 [(out-instruction i in) (+ i 2)])))
  ...
  ([input]
   (loop [[in i] [input 0]]
     (let [instruction (int (nth in i))]
       (cond
         (= instruction 99) in
         (= instruction 1) (recur [(op-instruction 1 i in 0 0 nil) (+ i 4)])
         (= instruction 2) (recur [(op-instruction 2 i in 0 0 nil) (+ i 4)])
         (= instruction 3) (recur [(in-instruction i in ) (+ i 2)])
         (= instruction 4) (recur [(out-instruction i in) (+ i 2)])
